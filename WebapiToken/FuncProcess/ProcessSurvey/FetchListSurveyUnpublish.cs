...
{
    public static class FetchListSurveyUnpublish
    {
        private static DBS db = new DBS();
        public static async Task<int> getTotalRecord()
        {
            var total = (from a in db.surveys where a.deleted == false && a.publish == false select a).Count();
            return total;
        }
        ...
        public static async Task<IQueryable<SurveyDetails>> getListSurveys(int skip_row, int page_size)
        {
            var list = (from a in db.surveys
                        ...
                        where a.surveys_type_id == b.id && a.deleted == false && a.publish == false
