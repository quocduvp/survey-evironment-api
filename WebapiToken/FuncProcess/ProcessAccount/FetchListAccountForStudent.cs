...
{
    public class FetchListAccountForStudent
    {
        private static DBS db = new DBS();
        public static async Task<int> getTotalRecord()
        {
            var total = (from a in db.accounts where a.role_id == 2 && a.status == true select a).Count();
            return total;
        }
        ...
        public static async Task<IQueryable<Lists>> getListAccount(int skip_row, int page_size)
        {
            ...
            var accounts = (from a in db.accounts
                            from b in db.roles
                            where a.role_id == b.id && a.role_id == 2 && a.status == true
