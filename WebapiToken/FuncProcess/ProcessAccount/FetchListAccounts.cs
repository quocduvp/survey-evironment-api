...
{
    public static class FetchListAccounts
    {
        private static DBS db = new DBS();
        public static async Task<int> getTotalRecord()
        {
            var total = (from a in db.accounts where a.status == true select a).Count();
            return total;
        }
        ...
        public static async Task<IQueryable<Lists>> getListAccount(int skip_row, int page_size)
        {
            ...
            var accounts = (from a in db.accounts
                            from b in db.roles
                            where a.role_id == b.id && a.status == true
                            orderby a.create_at ascending
                            select new Lists
                            {
                                id = a.id,
                                role_id = a.role_id,
